...
begin
  ...
  lxc_precise.initialize_commands << 'cat "" > /etc/default/lxc'
  lxc_precise.initialize_commands << '/opt/chef/embedded/bin/gem install attribute_struct --no-rdoc --no-ri'
  lxc_precise.initialize_commands << 'apt-get install lxc -o Dpkg::Options::="--force-confold" --no-install-recommends'
rescue => e
